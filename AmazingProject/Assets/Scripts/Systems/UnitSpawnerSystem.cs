...
public class UnitSpawnerSystem : JobComponentSystem
{
	...
	// to run on the newly-spawned entities before they're rendered for the first time, the UnitSpawnerSystem
	...
	struct SpawnJob : IJobForEachWithEntity<UnitSpawnerComponent, LocalToWorld>
	{
		public EntityCommandBuffer.Concurrent CommandBuffer;

		public void Execute(Entity entity, int index, [ReadOnly] ref UnitSpawnerComponent spawnerFromEntity,
			[ReadOnly] ref LocalToWorld location)
		{
			for (var x = 0; x < spawnerFromEntity.CountX; x++)
			{
				...
				{
					var instance = CommandBuffer.Instantiate(index, spawnerFromEntity.UnitPrefab);

					// Place the instantiated in a grid with some noise
					var position = math.transform(location.Value,
						new float3(x * 1.3F, 0f, y * 1.3F));
